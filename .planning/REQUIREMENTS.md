...
- [ ] **USER-05**: Super Admin pode remover o acesso de um usuário (soft-delete — remove o vínculo com o tenant/agência, mantém a conta Auth) com revogação de sessão imediata via `signOut('global')`, evitando o gap de JWT desatualizado (claims só são re-emitidas no login/refresh)
