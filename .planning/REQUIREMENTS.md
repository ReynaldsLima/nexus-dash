...
- [x] **AGENCY-08**: Tenant/agency-scoped write endpoints (starting with `PATCH /api/leads/[id]/status`) verify the caller's authorization server-side instead of trusting a client-supplied tenant identifier — FULLY satisfied as of Phase 6 Plan 03 (2026-07-11): `PATCH /api/leads/[id]/status` (Phase 5 Plan 08), `GET /api/leads` (Phase 6 Plan 02), and `POST /api/leads/chat` (Phase 6 Plan 03, also closes audit finding F3) all enforce the same `get_user_role()` role gate + `getClaims()`-sourced tenant/agency scope check.
