...
**Last action:** Blocker do Auth Hook resolvido via `/gsd-debug` (sessão `auth-hook-wired-to-wrong-function`, arquivada em `.planning/debug/resolved/`). Usuário trocou a seleção do Custom Access Token Hook no Supabase Dashboard (Authentication → Hooks) da Edge Function `custom-access-token` para `pg-functions://postgres/public/custom_access_token_hook`. Verificado via Management API (`hook_custom_access_token_uri` agora correto) e via teste real de sign-in (usuário de teste efêmero + row real de `tenant_users` no tenant `beta-test` → JWT `app_metadata` populado corretamente com `role: 'viewer'`, `tenant_id`, `tenant_slug: 'beta-test'`, sem a chave `slug` incorreta nem o valor `visualizador`). Usuário de teste e row de `tenant_users` removidos após verificação — nenhum dado real de tenant foi alterado. Anteriormente: Fase 05 Plano 02 executado — agency data layer (migrations `0017`/`0018`/`0019`, tabelas `agencies`/`agency_users`/`agency_tenants` com RLS + `get_agency_id()`, 5 policies `_agency_select`, `custom_access_token_hook` estendido com branch de agência; `tests/agency-rls.test.ts` com 8/8 testes passando).
**Stopped at:** Completed 05-02-PLAN.md; auth hook blocker resolved.
**Next action:** Retomar execução da Fase 05 a partir da Wave 3 (Plano 03 — Cliente role collapse, `tenant_users.role` para valor único) — o blocker do Auth Hook que recomendava pausa antes do Plano 04/07 está resolvido, login real de agência agora popula `role`/`agency_id` no JWT corretamente. Pendências não bloqueantes seguem: retomar Fase 2 (Data Pipeline, Plan 05) bloqueada até Google Ads Developer Token ser aprovado; autorar LEADS-01..LEADS-05 em REQUIREMENTS.md; commitar `app/[tenant-slug]/leads/agente/`/`app/api/leads/chat/`; corrigir os 4 erros de `tsc` pré-existentes listados em deferred-items.md antes do Plano 03 (role collapse) para evitar mascarar uma regressão de tipo real.
