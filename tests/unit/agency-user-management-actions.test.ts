...
describe('editAgencyUserEmail Server Action (USER-03)', () => {
  it.todo('editAgencyUserEmail calls admin.updateUserById with email + email_confirm:true')
  it.todo('editAgencyUserEmail rejects a non-super_admin caller with the auth-gate error')
})

describe('resetAgencyUserPassword Server Action (USER-04)', () => {
  it.todo('resetAgencyUserPassword calls admin.updateUserById with a >=16-char generated password')
  it.todo('resetAgencyUserPassword rejects a non-super_admin caller')
})

describe('removeAgencyUserAccess Server Action (USER-05)', () => {
  it.todo('removeAgencyUserAccess deletes the agency_users row scoped by BOTH agency_id AND user_id')
  it.todo('removeAgencyUserAccess calls revoke_user_sessions RPC with the target user_id')
  it.todo('removeAgencyUserAccess rejects a non-super_admin caller')
})
